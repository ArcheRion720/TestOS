...
static inline size_t vmm_setup_level(uintptr_t* level, size_t index, uintptr_t flags, struct pool_allocator* page_allocator)
{
    ...
    {
        uintptr_t frame = pool_fetch(page_allocator);
        if(!frame)
            return 0;
...
void vmm_map(struct vmm_mapping_desc desc, struct pool_allocator* page_allocator)
{
    ...
    size_t index = desc.paddr >> 12;

    for(size_t i = 0; i < levels; i++)
    {
        if(!vmm_setup_level(level_ptr, index & 0x1FF, desc.flags, page_allocator))
            return;

        level_ptr = (uintptr_t*)HH_ADDR(level_ptr[index & 0x1FF] & 0x0000FFFFF000);
        index >>= 9;
    }
...
void vmm_make_map(uintptr_t* pml4, uint16_t pcid, struct pool_allocator* page_allocator)
{
    ...
    uintptr_t* kernel_map = (uintptr_t*)HH_ADDR(&kernel_mappings);
